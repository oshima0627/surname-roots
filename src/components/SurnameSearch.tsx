  ...
  return (
    <div>
      <input
        ...
        aria-label="苗字を検索"
        className="w-full rounded-lg border border-keisen bg-surface px-4 py-3 text-lg outline-none focus:border-ai"
      />
      ...
      {results.length > 0 && (
        <ul className="mt-4 divide-y divide-keisen rounded-lg border border-keisen bg-surface">
          {results.map((entry) => (
            <li key={entry.slug}>
              <Link
