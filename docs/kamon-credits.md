...
| ファイル | 紋名 | 使用している苗字 | 作者 | ライセンス | 出典 | 改変 |
|---|---|---|---|---|---|---|
...
`umebachi`, `watanabe-boshi`）は、元のファイルが「不透明な地色の正方形／円の上に、紋の形を
反転色（白）で重ねて描く」構成だったため、単純な色置換では紋の形が見えなくなってしまう
（地の色だけが`currentColor`になり、紋そのものが透明の穴になる）。これを避けるため、
SVGの`<mask>`要素を使って元の重なり順を保持したまま再構成し、紋の形だけが`currentColor`で
塗られ、地の部分は透明になるようにした。
